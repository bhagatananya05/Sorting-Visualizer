...
async function selectionSort(barsArray) {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    barsArray[minIndex].style.background = "rgb(40,90,230)";
    await new Promise((resolve) => setTimeout(resolve, delay));

    for (let j = i + 1; j < n; j++) {
      // change the color of the bar to be compared to blue
      barsArray[j].style.background = "rgb(40,90,230)";
      await new Promise((resolve) => setTimeout(resolve, delay));
      ...
      if (
        ...
        barsArray[minIndex].style.background = "rgb(40,9 0,230)";
        await new Promise((resolve) => setTimeout(resolve, delay));
      }
    ...
    if (minIndex != i) await swap(barsArray[minIndex], barsArray[i]);
